...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd) {
	...
	// ウィンドウクラスの設定
	wc.lpszClassName = _T("ObjeqtNote_MainWindow");			// 独自に定義したウィンドウクラス"ObjeqtNote_MainWindow".
	...
	if (!RegisterClass(&wc)) {	// RegisterClassにwcをセットして登録.
		...
		// エラー処理
		MessageBox(NULL, _T("ウィンドウクラスの登録に失敗しました."), _T("ObjeqtNote"), MB_OK | MB_ICONHAND);	// MessageBoxで"ウィンドウクラスの登録に失敗しました!"と表示.
		return -1;	// 異常終了(-1)を返す.

	}

	// ウィンドウクラスの作成
	hWnd = CreateWindow(wc.lpszClassName, _T("ObjeqtNote"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowで"ObjeqtNote_MainWindow"ウィンドウクラスのウィンドウ"ObjeqtNote"を作成.
	if (hWnd == NULL) {

		// エラー処理
		MessageBox(NULL, _T("ウィンドウの作成に失敗しました!"), _T("ObjeqtNote"), MB_OK | MB_ICONHAND);	// MessageBoxで"ウィンドウの作成に失敗しました!"と表示.
		return -2;	// 異常終了(-2)を返す.
